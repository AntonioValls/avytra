...
4. **Columna lateral (sticky en escritorio, al final en móvil con barra fija inferior "Contactar"):**
   - **Contactar con el propietario**: nombre de contacto, método preferido destacado, resto de canales. Teléfono/WhatsApp tras clic "Mostrar" (Livewire, rate limit) para dificultar scraping. Desde Phase 11 el email no se muestra nunca: "Enviar mensaje" abre el formulario relay (`public.contact-request-form`), presente en toda publicación publicada (docs/12, ADR-019).
