...
Implementación (Phase 3): `pages::listings.wizard` con `?paso=N` en la URL, Form Objects en `App\Livewire\Forms\ListingWizard\*` (reutiliza `BusinessForm`, `LocationForm` y `OnlineProfileForm` en los pasos 2 y 5). Con una empresa existente el borrador se crea al completar el paso 1; con una empresa nueva, al completar el paso 2 (nombre y sector son obligatorios en base de datos). A partir de ahí la navegación entre pasos es libre y cada avance persiste. El paso 7 (Phase 6) incrusta el componente `businesses.images` (`resources/views/components/businesses/⚡images.blade.php`), el mismo que el formulario de empresa muestra al editar (al crear, un aviso: primero se crea la empresa). Cada imagen se guarda en cuanto se elige (no hay botón de guardar): `flux:file-upload` con progreso, validación estricta desde `config('avytra.media')`, `wire:sort` para el orden de la galería, alt editable por imagen, limitador `image-upload` por usuario y `wire:poll` mientras la cola genera las conversiones. Las tarjetas de "Mis empresas" muestran la portada. Desde Phase 4 la vista previa del paso 8 renderiza el parcial público real (`public/listings/partials/content`) a través de `PublicListingPresenter::withTitle()`, con el título que se está escribiendo. Las tarjetas de empresa enlazan con `?empresa=ID` para preseleccionarla.
